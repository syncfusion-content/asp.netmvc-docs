...
I> For foreign key column the sorting and grouping is based on the `ForeignKeyField` instead of `ForeignKeyValue`. You can refer [Foreign Key Adaptor](https://help.syncfusion.com/aspnetmvc/grid/data-adaptors#foreign-key-adaptor) to sort and group the foreign key column based on `ForeignKeyValue`.

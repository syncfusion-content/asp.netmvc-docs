...
A pop-up with a confirmation message will get displayed before deleting an appointment, which can be customized as mentioned [here](/aspnetmvc/schedule/globalization-and-localization#localization:localizing-specific-words).
...
* `DataSource` – Bind the categorize collection. This property should be assigned with the List data collection or remote data. We have below 6 default values for data source collection.
...
* `Template` – Customize the priority icon/images using template options. Refer this [priority template](/aspnetmvc/schedule/templates#priority-settings-template) topic.
* `DataSource` – binds the priority collection. This property should be assigned with the List data collection or remote data. We have below 4 default values for priority data source collection.
...
The public method `searchAppointment` is used to search the appointments in the scheduler dataSource. It contains below four arguments such as search string, search field, filter operator and ignorecase.
...
**predicate** – Add more than one condition query using **and**, **or** `predicate`.
